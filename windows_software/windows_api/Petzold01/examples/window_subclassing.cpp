...
WNDPROC button1_proc;
WNDPROC button2_proc;

LRESULT CALLBACK new_button_proc (HWND hwnd, UINT message,
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_CREATE:
		{
			...
			//---------------------------------------------------
			//Windows subclassing support
			button1_proc = (WNDPROC)SetWindowLong (button1.hwndButton,
    GWL_WNDPROC, (LONG)new_button_proc);
			button2_proc = (WNDPROC)SetWindowLong (button2.hwndButton,
    GWL_WNDPROC, (LONG)new_button_proc);
			//---------------------------------------------------
		}
			return 0 ;
    ...
    WPARAM wParam, LPARAM lParam)
{
	...
	WNDPROC old_address; //the OS's windows procedure
	HWND next_focus; //the next child window to get focus

	//figure out "old_address" and "next_focus"
	if(id == BUTTON1_ID)
	{
		old_address = button1_proc;
		next_focus = button2.hwndButton;
	}
	else if(id == BUTTON2_ID)
	{
		old_address = button2_proc;
		next_focus = button1.hwndButton;
	}
	...
	return CallWindowProc (old_address, hwnd, message, wParam, lParam);
}
